...
void setup() {
  ...
  nfcs = new OpenNFCSense4P(this, "tagProfile.csv", 300, 300, 500); 
...
void draw() {
  ...
  if (nbitList.size()>0) {
    ...
    if (lastString.equals("Hot wheels")){
      temp = nbitList.get(0).getFrequency();
  ...
  if (lastString.equals("E-fan")) {
    if (millis()-timer<1000) {
      if (lastFreq > 52) {
        ...
        drawString("Turning off...", 50, height/4+100);
      } else {
  ...
  if (lastString.equals("Hot wheels")) {
    if (millis()-timer>100) temp = 0; 
    pointerValue = pointerValue*0.9 + temp*0.1;
    drawSpeedmeter(pointerValue, 500, 150, 200, 200, 100); 
  }
  /*== Put your codes above ==*/
