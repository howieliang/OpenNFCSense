...
String[] QStrings = {"16 = ", "9 = ", "27 = "};
String[] AStrings = {"4C", "3C", "3D"};
int qIndex = 0;
...
void draw() {
  ...
  if (nbitList.size()>0) {
    ...
    for (int i = nbitList.size()-1; i >=0; i--) {
      ...
      if (nb.getName().equals("for")) tempString += "A";
      else if (nb.getName().equals("while")) tempString += "B";
  ...
  if (lastString.equals("")) {
    drawString("Output:", 50, 50, color(0),48);
    drawString("0", 50, 100, color(0),48);
    drawString("Code:", 450, 50, color(0),48);
    drawString("int i=0", 450, 100+0*24, color(0),24);
    drawString("{", 450, 100+1*24, color(0),24);
    drawString("  print(i+\',\');", 450, 100+2*24, color(0),24);
    drawString("}", 450, 100+3*24, color(0),24);
  } else {
    if (lastString.charAt(0)=='A') {
      if (lastString.length()>=2) {
        int n1 = lastString.charAt(1)-'0';
        int n2 = lastString.charAt(2)-'0';
        int n3 = lastString.charAt(3)-'0';
        String s = "";
        int i = 0;
        for (i=n1; i<n2; i+=n3){
          s += i;
        ...
        drawString("Output:", 50, 50, color(0),48);
        drawString(s, 50, 100, color(22, 160, 133),48);
        drawString("Code:", 450, 50, color(0),48);
        drawString("int i=0;", 450, 100+0*24, color(0),24);
        drawString("for ( i="+n1+" ; i<"+n2+"; i+="+n3+") {", 450, 100+1*24, color(22, 160, 133),24);
        drawString("  print(i+\',\');", 450, 100+2*24, color(0),24);
        drawString("}", 450, 100+3*24, color(22, 160, 133),24);
      }
    }
    if (lastString.charAt(0)=='B') {
      if (lastString.length()>=2) {
        int n1 = lastString.charAt(1)-'0';
        String s = "";
        int i = 0;  
        ...
        drawString("Code:", 450, 50, color(0),48);
        drawString("int i=0;", 450, 100+0*24, color(0),24);
        drawString("while (i<"+n1+") {", 450, 100+1*24, color(22, 160, 133),24);
        drawString("  print(i+\',\');", 450, 100+2*24, color(0),24);
        drawString("  i = i+1;", 450, 100+3*24, color(22, 160, 133),24);
        drawString("}", 450, 100+4*24, color(22, 160, 133),24);
      }
    }
  }
  /*== Put your codes above ==*/
...
void drawString(String s, int x, int y, color c, float t) { //print the latest tag appearance
  pushStyle();
  textAlign(LEFT, TOP);
  fill(c);
  textSize(t);
