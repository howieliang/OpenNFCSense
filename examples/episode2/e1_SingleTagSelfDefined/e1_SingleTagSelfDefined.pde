...
void draw() {
  ...
  if (nbitList.size()>0) {
    ...
    for (int i = nbitList.size()-1; i >=0; i--) {
      ...
      if (nb.getName().equals("power2")) tempString += "C";
      else if (nb.getName().equals("power3")) tempString += "D";
  ...
  if (lastString.equals("")) {
    drawString(QStrings[qIndex]+lastString, 50, 100, color(0));
  } else {
    if (lastString.equals(AStrings[qIndex])) bCorrect = true;
    else bCorrect = false;
    if (bCorrect) { 
      if (lastString.length()>=2) {
        int n1 = lastString.charAt(0)-'0'; 
        int o1 = lastString.charAt(1)-'A';
        drawString(QStrings[qIndex]+n1+(o1==2?'\u00B2':'\u00B3'), 50, 100, color(22, 160, 133));
      }
    } else { 
      if (lastString.length()>=2) {
        int n1 = lastString.charAt(0)-'0'; 
        int o1 = lastString.charAt(1)-'A';
        drawString(QStrings[qIndex]+n1+(o1==2?'\u00B2':'\u00B3'), 50, 100, color(231, 76, 60));
      }
    }
  }
  /*== Put your codes above ==*/
